...
// Diekspor agar panel admin selalu memuat seluruh produk (bukan hanya satu
// halaman DEFAULT_LIMIT) dan diagnostik dapat menyebut batas yang sama.
export const MAX_PRODUCTS = 50;
const DEFAULT_LIMIT = 20;

function normalizePrice(value) {
  const number = typeof value === 'number' ? value : Number(value);
  if (!Number.isSafeInteger(number) || number < 0 || number > 999_999_999_999) return null;
  return number;
}
...
export async function createProduct(botId, payload) {
  ...
  const existing = await getProducts(botId, { limit: MAX_PRODUCTS });
  if (existing.length >= MAX_PRODUCTS) throw new SettingsStorageError(`Maksimal ${MAX_PRODUCTS} produk dapat ditambahkan pada tahap ini.`);
...
export function catalogListText(products, title = '🛍 Katalog Produk') {
  if (!products.length) {
    return `${title}\n\nBelum ada produk yang tersedia saat ini. Silakan cek kembali nanti ya.`;
  }
  const lines = products.map((product) => `[${product.sortOrder}]${product.name} [${formatRupiah(product.price)}]`);
