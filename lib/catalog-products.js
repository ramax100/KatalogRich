...
export async function getProducts(botId, { activeOnly = false, categoryId = null, query = '', popular = false, featuredOnly = false, sortMode = 'default', limit = DEFAULT_LIMIT, offset = 0 } = {}) {
  ...
  if (featuredOnly) sort = 'sort_order.asc';
  else if (popular) sort = 'view_count.desc,sort_order.asc';
  else if (sortMode === 'price-low') sort = 'price.asc,sort_order.asc';
  else if (sortMode === 'price-high') sort = 'price.desc,sort_order.asc';
  else if (sortMode === 'name') sort = 'name.asc,sort_order.asc';
...
export function catalogListContent(products, title = '🛍 Katalog Produk', startNumber = 1, { boldProductNames = false } = {}) {
  if (!products.length) {
  ...
  products.forEach((product, index) => {
    ...
    text += `${number} | `;
    if (boldProductNames && product.name) {
      entities.push({ type: 'bold', offset: text.length, length: product.name.length });
    }
    text += `${product.name} | ${formatRupiah(product.price)}`;
    if (index < products.length - 1) text += '\n';
  });

  text += '\n\nBalas nomor produk untuk melihat detail.\nCari: /cari nama produk · Kategori: /kategori';
  const safeText = text.slice(0, 4096);
  const safeEntities = entities.filter((entity) => entity.offset + entity.length <= safeText.length);
  return { text: safeText, entities: safeEntities };
}

export function catalogListText(products, title = '🛍 Katalog Produk', startNumber = 1) {
  return catalogListContent(products, title, startNumber).text;
}
