...
export async function updateWelcomeText(botId, welcomeText, options = {}) {
  const fields = { welcome_text: welcomeText, updated_at: new Date().toISOString() };
  // Gambar welcome ikut disimpan hanya ketika opsi imageUrl diberikan secara
  // eksplisit (string URL atau null untuk menghapus) — simpan teks saja tidak
  // menyentuh kolom gambar sehingga aman walau migrasi kolom belum dijalankan.
  if (Object.hasOwn(options, 'imageUrl')) fields.welcome_image_url = options.imageUrl || null;
  const response = await supabaseRequest(`${TABLE}?bot_id=eq.${encodeURIComponent(String(botId))}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=representation' },
...
// Kirim foto ATAU GIF animasi dengan satu panggilan: GIF harus memakai
// sendAnimation (lewat sendPhoto, GIF tampil diam sebagai gambar biasa).
// Metode dipilih otomatis dari akhiran URL-nya.
export async function sendTelegramVisual(token, chatId, fileUrl, caption = '', replyMarkup = undefined) {
  const isGif = isGifUrl(fileUrl);
  const payload = { chat_id: chatId };
  payload[isGif ? 'animation' : 'photo'] = fileUrl;
  if (caption) payload.caption = String(caption).slice(0, 1024);
  if (replyMarkup) payload.reply_markup = replyMarkup;
  const { ok } = await telegramRequest(token, isGif ? 'sendAnimation' : 'sendPhoto', payload);
  return ok;
}
