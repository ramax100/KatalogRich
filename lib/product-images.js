...
function getImageData(imageData) {
  ...
  if (!buffer.length || buffer.length > MAX_IMAGE_BYTES) {
    throw new SettingsStorageError('Ukuran gambar maksimal 3 MB.');
  }
...
function publicImageUrl(pathname, version = '') {
  const { url } = getSupabaseServerConfig();
  const base = `${url}/storage/v1/object/public/${BUCKET}/${pathname}`;
  return version ? `${base}?v=${encodeURIComponent(String(version))}` : base;
}

function storeLogoPath(botId) {
  const id = Number(botId);
  if (!Number.isSafeInteger(id) || id < 1) throw new SettingsStorageError('Bot tidak valid.');
  // Path tanpa ekstensi agar logo bisa diganti JPG/PNG/WEBP/GIF tanpa perlu
  // kolom database untuk menyimpan ekstensi. Content-Type tetap disimpan oleh
  // Supabase Storage, sehingga browser dapat merendernya dengan benar.
  return `${id}/store-logo`;
}

export function getStoreLogoUrl(botId, version = '') {
  return publicImageUrl(storeLogoPath(botId), version);
}

export async function storeLogoExists(botId) {
  const { url } = getSupabaseServerConfig();
  const response = await fetch(`${url}/storage/v1/object/public/${BUCKET}/${storeLogoPath(botId)}`, {
    method: 'HEAD'
  }).catch(() => null);
  return Boolean(response?.ok);
}

export async function uploadStoreLogo(botId, imageData) {
  const image = getImageData(imageData);
  if (!image) return null;
  await ensureImageBucket();

  const pathname = storeLogoPath(botId);
  const response = await storageRequest(`object/${BUCKET}/${pathname}`, {
    method: 'POST',
    headers: {
      'Content-Type': image.mimeType,
      // Jangan immutable karena path logo tetap dan dapat diganti dari admin.
      'Cache-Control': '60, must-revalidate',
      'x-upsert': 'true'
    },
    body: image.buffer
  });
  if (!response.ok) throw new SettingsStorageError('Logo toko belum dapat diunggah.');
  return getStoreLogoUrl(botId, Date.now());
}

export async function deleteStoreLogo(botId) {
  await ensureImageBucket().catch(() => null);
  const response = await storageRequest(`object/${BUCKET}/${storeLogoPath(botId)}`, { method: 'DELETE' }).catch(() => null);
  return Boolean(response?.ok);
}
