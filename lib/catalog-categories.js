...
const PRODUCT_TABLE = 'catalog_products';
const MAX_CATEGORIES = 30;
const MAX_PRODUCTS_FOR_CATEGORY_COUNTS = 5000;
...
export async function getCategoryProductCountRows(botId) {
  const response = await supabaseRequest(
    `${PRODUCT_TABLE}?bot_id=eq.${encodeURIComponent(String(botId))}&is_active=eq.true&select=category_id&limit=${MAX_PRODUCTS_FOR_CATEGORY_COUNTS}`
  );
  const rows = await response.json();
  return Array.isArray(rows)
    ? rows.map((row) => ({ categoryId: row.category_id ? Number(row.category_id) : null }))
    : [];
}
