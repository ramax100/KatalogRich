...
// 'products:popular', atau 'products:search:QUERY'). KONTEKS BERLAKU TANPA
// BATAS WAKTU — ia selalu ditimpa setiap kali bot menampilkan menu baru,
// sehingga angka polos selalu mengikuti layar terakhir yang benar-benar dilihat
// customer.
function normalizeMenuContext(context) {
  const value = String(context || '').trim();
  if (value === 'categories' || value === 'products' || value === 'products:popular') return value;
