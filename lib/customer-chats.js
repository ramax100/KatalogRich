...
// daftar kategori konteksnya 'categories', setelah daftar/detail produk
// konteksnya 'products'. KONTEXS BERLAKU TANPA BATAS WAKTU — ia selalu ditimpa
// setiap kali bot menampilkan menu baru, sehingga selalu mengikuti layar
// terakhir yang benar-benar dilihat customer.
export async function setChatMenuContext(botId, chatId, context) {
  const normalized = normalizeChatId(chatId);
  if (botId === undefined || botId === null || !normalized) return;
  if (!['categories', 'products'].includes(context)) return;
  ...
  await supabaseRequest(
    ...
    {
      method: 'PATCH',
      body: JSON.stringify({ menu_context: context, menu_context_at: new Date().toISOString() })
    }
  );
}
...
export async function getChatMenuContext(botId, chatId) {
  ...
  return { context: rows[0].menu_context || null, at: rows[0].menu_context_at || null };
}
